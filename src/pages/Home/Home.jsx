...
const Home = () => {
  ...
  return (
    <main>
      ...
      {/* Affichage des éléments Carte Logements sur la page d'acceuil via les résultats du Fetch, on assigne une key aux éléments de la liste data.map */}

      <section className="cards">
        {data.map((logements) => (
          <article className="logements" key={logements.id}>
            <NavLink to={"/logements/" + logements.id} className="linkCard">
              <img
                src={logements.cover}
                alt={logements.title}
